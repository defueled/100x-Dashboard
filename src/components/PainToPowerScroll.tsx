...
type PainPoint = {
    ...
    title: string;
    description: string;
    painIcon: typeof TrendingDown;
    painColor: string;
    painBg: string;
    solutionTitle: string;
    solutionDescription: string;
    solutionIcon: typeof TrendingUp;
...
const VISUAL = [
    {
        id: "inflation",
        painIcon: TrendingDown, painColor: "text-rose-500", painBg: "bg-rose-500/10",
        solutionIcon: TrendingUp, solutionColor: "text-brand-orange", solutionBg: "bg-brand-orange/10", solutionBorder: "border-brand-orange/30",
    },
    {
        id: "scams",
        painIcon: ShieldAlert, painColor: "text-amber-500", painBg: "bg-amber-500/10",
        solutionIcon: ShieldCheck, solutionColor: "text-brand-green", solutionBg: "bg-brand-green/10", solutionBorder: "border-brand-green/30",
    },
    {
        id: "ai_fear",
        painIcon: BotOff, painColor: "text-slate-500", painBg: "bg-slate-500/10",
        solutionIcon: Sparkles, solutionColor: "text-brand-blue", solutionBg: "bg-brand-blue/10", solutionBorder: "border-brand-blue/30",
    },
] as const;
...
const COPY = {
    lv: {
        section: {
            tag: "Kāpēc tieši tagad?",
            titleA: "No problēmas līdz",
            titleB: "priekšrocībai",
            subtitle: "Mēs izprotam lielākos šķēršļus un bailes mūsdienu finanšu pasaulē. 100x ekosistēma ir tavs vairogs pret neziņu un vērtības zudumu.",
            hoverHint: "Uzbīdi lai atrisinātu",
            solutionLabel: "100x Risinājums",
            perksHeader: "Praksē apgūsi:",
            ctaPrefix: "Aktivizēt",
            ctaSuffix: "Zināšanas",
        },
        painAnim: {
            inflationLabel: "-8.4% INFLĀCIJA",
            inflationSub: "Iekrājumi Deg",
            scamLabel: "RUGPULL",
            scamSub: "-99.9% PORTFELIS",
            aiLabel: "AI AIZVIETO",
            aiSub: "Cilvēka Vērtības Zudums",
        },
        powerAnim: {
            tradFiLabel: "SALIKTIE PROCENTI",
            tradFiSub: "S&P 500 / DIVIDENDES",
            deFiLabel: "AUDITĒTI LĪGUMI",
            deFiSub: "DECENTRALIZĒTAS FINANSES",
            aiSub: "10x Produktivitāte",
        },
        points: [
            {
                ...
                perks: ["Brokeru un platformu izvēle", "ETF un Akciju pamati", "Pasīvā ienākuma stratēģijas"],
            },
            {
                ...
                perks: ["Drošība un Wallets (Maki)", "Smart Contracts (Gudrie Līgumi)", "Vērtības un tirgus analīze"],
            },
            {
                ...
                perks: ["Prompt Inženierija un ChatGPT", "Satura Radīšanas Automatizācija", "Biznesa Darbplūsmu (Workflows) pielāgošana"],
            },
        ],
    },
    en: {
        section: {
            tag: "Why right now?",
            titleA: "From problem to",
            titleB: "advantage",
            subtitle: "We understand the biggest blockers and fears in modern finance. The 100x ecosystem is your shield against uncertainty and loss of value.",
            hoverHint: "Hover to solve",
            solutionLabel: "100x Solution",
            perksHeader: "What you'll learn:",
            ctaPrefix: "Unlock",
            ctaSuffix: "knowledge",
        },
        painAnim: {
            inflationLabel: "-8.4% INFLATION",
            inflationSub: "Savings on fire",
            scamLabel: "RUGPULL",
            scamSub: "-99.9% PORTFOLIO",
            aiLabel: "AI REPLACES YOU",
            aiSub: "Loss of human value",
        },
        powerAnim: {
            tradFiLabel: "COMPOUND INTEREST",
            tradFiSub: "S&P 500 / DIVIDENDS",
            deFiLabel: "AUDITED CONTRACTS",
            deFiSub: "DECENTRALISED FINANCE",
            aiSub: "10x productivity",
        },
        points: [
            {
                title: "Money loses value",
                description: "Inflation eats your savings. Wages don't keep up with grocery prices, and a bank savings account gives no real return.",
                solutionTitle: "TradFi & ETFs",
                solutionDescription: "We teach you how the market actually works. You'll learn to open an account with a trusted broker and invest into S&P 500 ETFs or dividends — turning cash into an asset that works for you.",
                perks: ["Choosing brokers and platforms", "ETF and stock fundamentals", "Passive-income strategies"],
            },
            {
                title: "Crypto feels like a scam",
                description: "Media only reports collapses or meme coins. Fear and FOMO make it easy to make mistakes and lose everything to market manipulation.",
                solutionTitle: "Web3 & DeFi",
                solutionDescription: "We teach you to tell fake projects apart from real blockchain protocols with lasting value. You'll learn self-custody wallets and decentralised finance (DeFi).",
                perks: ["Security & wallets", "Smart contracts", "Value and market analysis"],
            },
            {
                title: "AI will take my job",
                description: "Automation is sweeping every industry. ChatGPT seems to write code and copy better than people, creating real uncertainty about your future as an employee.",
                solutionTitle: "AI tools",
                solutionDescription: "AI won't take your job. Another person who knows how to use it will. 100x teaches the fundamentals of prompt engineering and workflow automation.",
                perks: ["Prompt engineering & ChatGPT", "Content creation automation", "Tuning business workflows"],
            },
        ],
    },
} as const;
...
const InflationPain = ({ label, sub }: { label: string; sub: string }) => (
    <motion.div
    ...
    >
        ...
        <div className="text-rose-500/80 font-bold text-xs uppercase mt-2 z-10 tracking-widest bg-rose-950/50 px-3 py-1 rounded-full backdrop-blur-sm">{sub}</div>
    </motion.div>
);

const TradFiPower = ({ label, sub }: { label: string; sub: string }) => (
    <motion.div
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    >
        ...
        >
            <TrendingUp size={32} /> {label}
        </motion.div>
        <div className="text-brand-orange/80 text-xs font-bold mt-2 z-10 tracking-widest bg-brand-orange/10 px-3 py-1 rounded-full border border-brand-orange/20">{sub}</div>
        <motion.div
...
const CryptoScamPain = ({ label, sub }: { label: string; sub: string }) => (
    <motion.div
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    >
        ...
        <div className="text-amber-500/80 font-bold text-xs uppercase mt-2 z-10 tracking-widest bg-amber-950/50 px-3 py-1 rounded-full">{sub}</div>
    </motion.div>
);

const DeFiPower = ({ label, sub }: { label: string; sub: string }) => (
    <motion.div
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    >
        ...
        >
            <ShieldCheck size={36} /> {label}
        </motion.div>
        <div className="text-brand-green/80 font-bold text-xs mt-2 z-10 tracking-widest bg-brand-green/10 px-3 py-1 rounded-full border border-brand-green/20">{sub}</div>
...
const AIFearPain = ({ label, sub }: { label: string; sub: string }) => (
    <motion.div
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    >
        ...
        <div className="text-slate-500/80 font-bold text-[10px] mt-2 z-10 tracking-widest uppercase">{sub}</div>
    </motion.div>
);

const AIPower = ({ sub }: { sub: string }) => (
    <motion.div
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    >
        ...
        <div className="text-brand-blue/80 font-bold text-xs mt-3 z-10 tracking-widest uppercase px-3 py-1 bg-brand-blue/10 rounded-full border border-brand-blue/10">{sub}</div>
...
export function PainToPowerScroll() {
    ...
    const painPoints: PainPoint[] = VISUAL.map((v, i) => ({
        ...v,
        ...copy.points[i],
    }));
        ...
        <section className="py-24 px-6 md:px-0 max-w-6xl mx-auto relative z-10">
            <ScrollReveal variant="blurFadeIn" className="text-center mb-16">
                ...
                <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-brand-dark mb-4">
                    {copy.section.titleA} <span className="text-gradient-premium">{copy.section.titleB}</span>
                </h2>
                        ...
                        <div className={`relative glass-card bg-white/90 rounded-[2rem] p-8 md:p-12 border-2 ${isHovered ? activeData.solutionBorder : 'border-gray-200'} shadow-premium transition-all duration-500 overflow-hidden`}>
                                ...
                                <AnimatePresence mode="wait">
                                    {!isHovered ? (
                                        <motion.div key="pain" className="w-full h-full absolute inset-0">
                                            {activeIndex === 0 && <InflationPain label={copy.painAnim.inflationLabel} sub={copy.painAnim.inflationSub} />}
                                            {activeIndex === 1 && <CryptoScamPain label={copy.painAnim.scamLabel} sub={copy.painAnim.scamSub} />}
                                            {activeIndex === 2 && <AIFearPain label={copy.painAnim.aiLabel} sub={copy.painAnim.aiSub} />}
                                        </motion.div>
                                    ) : (
                                        <motion.div key="power" className="w-full h-full absolute inset-0">
                                            {activeIndex === 0 && <TradFiPower label={copy.powerAnim.tradFiLabel} sub={copy.powerAnim.tradFiSub} />}
                                            {activeIndex === 1 && <DeFiPower label={copy.powerAnim.deFiLabel} sub={copy.powerAnim.deFiSub} />}
                                            {activeIndex === 2 && <AIPower sub={copy.powerAnim.aiSub} />}
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                                    ...
                                    {!isHovered && (
                                        ...
                                        >
                                            <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-full border border-gray-200 shadow-sm flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest whitespace-nowrap">{copy.section.hoverHint}</span>
                                            </div>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            ...
                            <div className="bg-gray-50 rounded-2xl p-5 mb-8">
                                <h4 className="text-brand-dark font-bold text-sm mb-3 flex items-center gap-2">
                                    <Wallet size={16} className="text-brand-dark/40" /> {copy.section.perksHeader}
                                </h4>
                            ...
                            <button
                                ...
                                {copy.section.ctaPrefix} {activeData.solutionTitle} {copy.section.ctaSuffix} <ArrowRight size={18} className={isHovered ? "animate-pulse" : ""} />
                            </button>
